...
---
---@field _entitiesToBeDeconstructed_deduped Task_ScanAreasForActionsToComplete_EntitiesDeduped @ A table of all the raw entities (deduped) needing to be deconstructed across all areas merged together.
---@field _natureToBeDeconstructed_deduped Task_ScanAreasForActionsToComplete_EntitiesDeduped @ A table of all the trees and rocks (deduped) needing to be deconstructed across all areas merged together. These are marked for deconstruction by any force and may belong to the robots force and thus included already in _entitiesToBeDeconstructed_raw. Post deduping this list is checked for force and duplicates and merged in to _entitiesToBeDeconstructed_deduped ready for actual usage.
---@field _entitiesToBeUpgraded_deduped Task_ScanAreasForActionsToComplete_EntitiesDeduped @ A table of all the raw entities (deduped) needing to be upgraded across all areas merged together.
---@field _ghostsToBeBuilt_deduped Task_ScanAreasForActionsToComplete_EntitiesDeduped @ A table of all the raw entities (deduped) needing to be built across all areas merged together.
...
---
---@field entitiesToBeDeconstructed Task_ScanAreasForActionsToComplete_EntitiesToBeActioned
---@field entitiesToBeUpgraded Task_ScanAreasForActionsToComplete_EntitiesToBeActioned
---@field ghostsToBeBuilt Task_ScanAreasForActionsToComplete_EntitiesToBeActioned
...
---@alias Task_ScanAreasForActionsToComplete_EntitiesDeduped table<Task_ScanAreasForActionsToComplete_EntityIdentifier, LuaEntity> @ A single table of all the raw entities (deduped) across all areas needing to be handled for their specific action type.
---@alias Task_ScanAreasForActionsToComplete_EntitiesToBeActioned table<Task_ScanAreasForActionsToComplete_EntityIdentifier, Task_ScanAreasForActionsToComplete_EntityDetails> @ A single table of all the entities across all areas needing to be handled for their specific action type.
...
---@alias Task_ScanAreasForActionsToComplete_EntityIdentifier string|uint # The entities unit_number or "destroyedId_[UNIQUE_NUMBER_PER_ENTITY]".
...
---@field toBeDeconstructedEntityDetails table<Task_ScanAreasForActionsToComplete_EntityIdentifier, Task_ScanAreasForActionsToComplete_EntityDetails>
---@field toBeDeconstructedEntities table<Task_ScanAreasForActionsToComplete_EntityIdentifier, LuaEntity>
---@field toBeUpgradedTypes table<string, table<Task_ScanAreasForActionsToComplete_EntityIdentifier, Task_ScanAreasForActionsToComplete_EntityDetails>> # Grouped by the entity item used to upgrade it first.
---@field toBeBuiltTypes table<string, table<Task_ScanAreasForActionsToComplete_EntityIdentifier, Task_ScanAreasForActionsToComplete_EntityDetails>> # Grouped by the entity item type used to build it first.

---@class Task_ScanAreasForActionsToComplete_EntityDetails
---@field identifier Task_ScanAreasForActionsToComplete_EntityIdentifier
...
ScanAreasForActionsToComplete._ProcessDedupedTableToProcessedTable = function(taskData, actionType, entitiesHandled)
    ...
    for identifier, entity in pairs(dedupedTable) do
        ...
        finalTable[entityDetails.identifier] = entityDetails

        -- Record the EntityDetails in to the Chunk Details. Some of these are grouped.
        if actionType == "deconstruct" then
            chunkDetails.toBeDeconstructedEntityDetails[entityDetails.identifier] = entityDetails
            chunkDetails.toBeDeconstructedEntities[entityDetails.identifier] = entityDetails.entity
        elseif actionType == "upgrade" then
            chunkDetails.toBeUpgradedTypes[entity_name] = chunkDetails.toBeUpgradedTypes[entity_name] or {}
            chunkDetails.toBeUpgradedTypes[entity_name][entityDetails.identifier] = entityDetails
        elseif actionType == "build" then
            chunkDetails.toBeBuiltTypes[entity_name] = chunkDetails.toBeBuiltTypes[entity_name] or {}
            chunkDetails.toBeBuiltTypes[entity_name][entityDetails.identifier] = entityDetails
        else
            error("unsupported action type")
        end
