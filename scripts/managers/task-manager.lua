--[[
    ...
    All Tasks are required to implement Task_Interface and Task_Details within their bespoke classes.
...
---@field RemovingRobotFromTask fun(thisTask:Task_Details, robot:Robot) # Called when a specific robot is being removed from a task. Removes any robot specific task data and any robot related task globals or ongoing activities will be stopped. This will propagates down to all sub tasks.
---@field PausingRobotForTask fun(thisTask:Task_Details, robot:Robot) # Called when a robot has been paused and so all of its activities within the job's tasks need to pause. This will propagates down to all sub tasks to pause active effects for this robot.
...
---@param task Task_Details # The Task that this robot specific data is for.
---@return TaskData_Robot
