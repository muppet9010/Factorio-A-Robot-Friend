...
---@field Remove fun(thisTask:Task_Data) # Called to remove a task. This will propagates down to all sub tasks to tidy up any non task managed globals and other active effects. TODO: needs to be called with care now we are 1 tack hierarchy per job.
...
---@field currentTaskIndex int # The current task in the `tasks` list that is the active task. This is for all robots. Some individual robots will be on different current task as recorded under the robotsTaskData.
...
TaskManager.CreateGenericTask = function(taskName, job, parentTask)
    ---@type Task_Data
    local task = { taskName = taskName, taskData = {}, robotsTaskData = {}, state = "active", tasks = {}, job = job, parentTask = parentTask }
    return task
end
